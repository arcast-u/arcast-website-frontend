...
const NavigationBar= () => {
  ...
  return (
    <>
      ...
      <nav className="flex w-full justify-between items-center px-[21px] md:px-10 lg:pt-4 lg:pb-5 3xl:px-[100px] 3xl:pt-6 py-2 3xl:pb-8 text-base font-medium " aria-label="Main Navigation">
        <div className={`${isOpen || isStudioOpen ? 'bg-[#FCFCFC] rounded-t-xl pt-3 pb-2 shadow-sm px-6' : 'bg-transparent'} flex relative w-full mx-auto justify-between gap-6 items-center self-stretch my-auto max-md:max-w-full`}>
              ...
              <div className="flex items-center justify-between lg:justify-stretch self-stretch ">
                  ...
                  <div onClick={bookSession} 
                  className={`w-fit ml-4 xl:ml-14 3xl:ml-16 hidden 
                  hover:scale-105 lg:flex rounded-lg bg-gradient-to-b 
                  from-[#FFC49D] ring-2 ${ pathname === '/' || pathname === '/about-us' ? "ring-[#64391E]" : "ring-[#FFEDE1]"} to-[#FF8C42] 
                  ...
                  <button onClick={handleDropdown} className='flex cursor-none hover:scale-105 3xl:ml-3 ml-2 lg:hidden items-center gap-1 self-stretch '>
                      <IoIosMenu className={`size-6 stroke-[1.5px] ${isOpen || isStudioOpen || pathname !== '/' && pathname !== '/about-us' ? 'text-[#333333]' : 'text-[#FCFCFC]'}`}/>
                  </button>
              </div>
            {isOpen && (
