  ...
  return (
    <div className='flex flex-col whitespace-nowrap'>
      <div className={`w-full ${hasDropdown ? '' : 'self-stretch'}`}>
        ...
        className={`flex justify-center cursor-none ${isStudioOpen === true || pathname === '/packages' 
        ? "text-[#333333]" 
        ...
        {(isActive || (isStudioOpen && hasDropdown)) && (
            <div className={`w-full border border-solid ${isStudioOpen && hasDropdown || pathname === '/packages' ? "bg-[#333333] border-[#333333]" : "bg-[#FCFCFC] border-[#FCFCFC]"} min-h-[1.5px]`} aria-hidden="true" />
        )}
      </div>
    </div>
  );
