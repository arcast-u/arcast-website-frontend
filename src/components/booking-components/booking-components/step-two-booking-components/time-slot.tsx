...
const TimeSlot = ({
  ...
  const [noConsecutiveTimeMessage, setNoConsecutiveTimeMessage] =
  ...
  const checkConsecutiveTimeSlots = React.useCallback(() => {
    ...
    const selectedIndex = sortedTimes.findIndex(
      (time) => time.start === selectedTime
    );
    ...
    const slotDuration =
      new Date(sortedTimes[selectedIndex].end).getTime() -
          ...
          <button
            ...
            className={`flex 3xl:px-6 3xl:py-5 lg:px-4 lg:py-2 justify-center p-3 my-auto cursor-pointer rounded-xl bg-neutral-100 ${
              time.start === selectedTime
                ? 'border border-[#989898] shadow-lg shadow-[#80808030]'
                : ''
            }`}
          >
